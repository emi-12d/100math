...
def generate_asymmetric_number_of_characters_Hundred_Square_Calculations_pdf(
    operator: str = "+", left_digits: int = 1, top_digits: int = 1
) -> None:
    ...
    for i in range(10):
        for j in range(10):
            ...
            if operator == "+":
                ...
                answers[i][j] = l_num / t_num if t_num != 0 else " - "
...
def generate_symmetric_number_of_characters_Hundred_Square_Calculations_pdf(
    operator="+", digits=1
):
    """
    ...
    （内部的に非対称関数を呼び出します）
    """
    generate_asymmetric_number_of_characters_Hundred_Square_Calculations_pdf(
        operator, left_digits=digits, top_digits=digits
    )
...
if __name__ == "__main__":
    # 【非対称】左2桁、上1桁の足し算
    generate_asymmetric_number_of_characters_Hundred_Square_Calculations_pdf(
        operator="+", left_digits=2, top_digits=1
    )

    # 【非対称】左3桁、上2桁の引き算
    generate_asymmetric_number_of_characters_Hundred_Square_Calculations_pdf(
        operator="-", left_digits=3, top_digits=2
    )

    # 【非対称】左2桁、上2桁の引き算
    generate_asymmetric_number_of_characters_Hundred_Square_Calculations_pdf(
        operator="-", left_digits=2, top_digits=2
    )

    # 【対称】2桁の掛け算 (対称関数を呼び出し)
    generate_symmetric_number_of_characters_Hundred_Square_Calculations_pdf(
        operator="*", digits=2
    )
